package com.kunbox.singbox.repository
...
import com.kunbox.singbox.core.SingBoxCore
import com.kunbox.singbox.ipc.SingBoxRemote
import com.kunbox.singbox.ipc.VpnStateStore
import com.kunbox.singbox.model.*
import com.kunbox.singbox.service.SingBoxService
import com.kunbox.singbox.service.ProxyOnlyService
import com.kunbox.singbox.utils.parser.Base64Parser
import com.kunbox.singbox.utils.parser.NodeLinkParser
import com.kunbox.singbox.utils.parser.SingBoxParser
import com.kunbox.singbox.repository.config.OutboundFixer
import com.kunbox.singbox.repository.config.InboundBuilder
import com.kunbox.singbox.repository.config.NodeLinkExporter
import com.kunbox.singbox.utils.parser.SubscriptionManager
import com.kunbox.singbox.database.AppDatabase
import com.kunbox.singbox.database.entity.ProfileEntity
import com.kunbox.singbox.database.entity.ActiveStateEntity
import com.kunbox.singbox.database.entity.NodeLatencyEntity
import java.io.File
...
import com.kunbox.singbox.utils.NetworkClient
import com.kunbox.singbox.utils.StringBuilderPool
import com.kunbox.singbox.utils.dns.DnsResolver
import com.kunbox.singbox.utils.dns.DnsResolveStore
...
class ConfigRepository(private val context: Context) {
    ...
    companion object {
        ...
        private const val DELETED_NODES_MMKV_ID = "deleted_node_tags_memory"
        private const val KEY_DELETED_NODE_TAGS = "deleted_node_tags"
        ...
        private val TYPE_DELETED_NODE_TAGS = object : TypeToken<Map<String, List<String>>>() {}.type
    ...
    private fun getProxyClient(): okhttp3.OkHttpClient? {
        val settings = cachedSettings ?: AppSettings()
        if (!com.kunbox.singbox.ipc.VpnStateStore.getActive() || settings.proxyPort <= 0) {
            return null
        }
        val timeout = settings.subscriptionUpdateTimeout.toLong()
    ...
    private val subscriptionManager = SubscriptionManager(listOf(
        SingBoxParser(gson),
        com.kunbox.singbox.utils.parser.ClashYamlParser(),
    ...
    // 删除节点持久化：按 profileId 记录被用户删除（屏蔽）的节点 tag
    // 目标：订阅更新后依然过滤这些垃圾节点，避免“删了又回来”。
    private val deletedNodeTagsByProfile = mutableMapOf<String, MutableSet<String>>()
    private val deletedNodeTagsMmkv: MMKV by lazy {
        MMKV.mmkvWithID(DELETED_NODES_MMKV_ID, MMKV.SINGLE_PROCESS_MODE)
    }
    ...
    private fun loadDeletedNodeTagsMemory() {
        val raw = deletedNodeTagsMmkv.decodeString(KEY_DELETED_NODE_TAGS, null)
        if (raw.isNullOrBlank()) return

        runCatching {
            @Suppress("UNCHECKED_CAST")
            gson.fromJson<Map<String, List<String>>>(raw, TYPE_DELETED_NODE_TAGS)
        }.getOrNull()?.let { saved ->
            synchronized(deletedNodeTagsByProfile) {
                deletedNodeTagsByProfile.clear()
                saved.forEach { (profileId, tags) ->
                    if (profileId.isBlank()) return@forEach
                    val safeTags = tags
                        .filter { it.isNotBlank() }
                        .toMutableSet()
                    if (safeTags.isNotEmpty()) {
                        deletedNodeTagsByProfile[profileId] = safeTags
                    }
                }
            }
        }
    ...
    private fun saveDeletedNodeTagsMemory() {
        val snapshot = synchronized(deletedNodeTagsByProfile) {
            deletedNodeTagsByProfile.mapValues { entry ->
                entry.value.toList().sorted()
            }
        }
        deletedNodeTagsMmkv.encode(KEY_DELETED_NODE_TAGS, gson.toJson(snapshot))
    }

    private fun getDeletedNodeTags(profileId: String): Set<String> {
        return synchronized(deletedNodeTagsByProfile) {
            deletedNodeTagsByProfile[profileId]?.toSet() ?: emptySet()
        }
    }

    private fun markNodeTagDeleted(profileId: String, tag: String) {
        if (profileId.isBlank() || tag.isBlank()) return
        var changed = false
        synchronized(deletedNodeTagsByProfile) {
            val bucket = deletedNodeTagsByProfile.getOrPut(profileId) { mutableSetOf() }
            changed = bucket.add(tag)
        }
        if (changed) {
            saveDeletedNodeTagsMemory()
        }
    }

    private fun removeNodeTagFromDeleted(profileId: String, tag: String) {
        if (profileId.isBlank() || tag.isBlank()) return
        var changed = false
        synchronized(deletedNodeTagsByProfile) {
            val bucket = deletedNodeTagsByProfile[profileId] ?: return@synchronized
            changed = bucket.remove(tag)
            if (bucket.isEmpty()) {
                deletedNodeTagsByProfile.remove(profileId)
            }
        }
        if (changed) {
            saveDeletedNodeTagsMemory()
    ...
    private fun clearDeletedNodeTagsForProfile(profileId: String) {
        if (profileId.isBlank()) return
        val changed = synchronized(deletedNodeTagsByProfile) {
            deletedNodeTagsByProfile.remove(profileId) != null
        }
        if (changed) {
            saveDeletedNodeTagsMemory()
        }
    ...
    private fun applyDeletedNodeFilter(profileId: String, config: SingBoxConfig): SingBoxConfig {
        val deletedTags = getDeletedNodeTags(profileId)
        if (deletedTags.isEmpty()) return config

        val outbounds = config.outbounds ?: return config
        val removedTags = outbounds
            .asSequence()
            .filter { it.type in PROXY_OUTBOUND_TYPES }
            .map { it.tag }
            .filter { it in deletedTags }
            .toSet()
        if (removedTags.isEmpty()) return config

        val filteredOutbounds = outbounds
            .filterNot { it.type in PROXY_OUTBOUND_TYPES && it.tag in removedTags }
            .toMutableList()
        val existingTags = filteredOutbounds.map { it.tag }.toSet()

        val sanitizedOutbounds = filteredOutbounds.map { outbound ->
            if (outbound.type == "selector" || outbound.type == "urltest" || outbound.type == "url-test") {
                val validRefs = outbound.outbounds.orEmpty().filter { it in existingTags }
                val fallbackRef = when {
                    validRefs.isNotEmpty() -> validRefs.first()
                    "direct" in existingTags -> "direct"
                    else -> existingTags.firstOrNull()
                }
                val safeRefs = if (validRefs.isNotEmpty()) validRefs else listOfNotNull(fallbackRef)
                val safeDefault = outbound.default?.takeIf { it in safeRefs } ?: safeRefs.firstOrNull()
        ...
        Log.i(TAG, "Filtered ${removedTags.size} deleted node(s) for profile=$profileId")
        return config.copy(outbounds = sanitizedOutbounds)
    }
    ...
    private fun loadConfig(profileId: String): SingBoxConfig? {
        ...
        return try {
            ...
            val deduplicated = deduplicateTags(config)
            val filtered = applyDeletedNodeFilter(profileId, deduplicated)
            // 如有删除过滤生效，回写配置文件，避免下次重复处理。
            if (filtered !== deduplicated) {
                runCatching {
                    configFile.writeText(gson.toJson(filtered))
                }
            }
            config = filtered
    ...
    private fun parseClashYamlConfig(content: String): SingBoxConfig? {
        return com.kunbox.singbox.utils.parser.ClashYamlParser().parse(content)
    }
    ...
    suspend fun importFromSubscription(
        ...
        try {
            ...
            val deduplicatedConfig = applyDeletedNodeFilter(profileId, deduplicateTags(config))
            ...
            if (autoUpdateInterval > 0) {
                com.kunbox.singbox.service.SubscriptionAutoUpdateWorker.schedule(context, profileId, autoUpdateInterval)
            }
    ...
    private suspend fun extractNodesFromConfig(
        ...
        val validOutbounds = outbounds.filter {
            it.type in PROXY_OUTBOUND_TYPES && it.tag !in detourTags && it.tag !in deletedTags
        }
    ...
    private fun containsFlagEmoji(str: String): Boolean {
        return com.kunbox.singbox.utils.RegionDetector.containsFlagEmoji(str)
    }
    ...
    private fun detectRegionFlag(name: String): String {
        return com.kunbox.singbox.utils.RegionDetector.detect(name)
    }
                        ...
                        if (!VpnStateStore.shouldTriggerPrepareRestart(1500L)) {
                            ...
                            val prepareIntent = if (coreMode == VpnStateStore.CoreMode.PROXY) {
                                Intent(context, ProxyOnlyService::class.java).apply {
                                    ...
                                    putExtra(
                                        com.kunbox.singbox.service.SingBoxService.EXTRA_PREPARE_RESTART_REASON,
    ...
    fun deleteProfile(profileId: String) {
        // 取消自动更新任务
        com.kunbox.singbox.service.SubscriptionAutoUpdateWorker.cancel(context, profileId)
        clearDeletedNodeTagsForProfile(profileId)
    ...
    fun importProfileDirectly(profile: ProfileUi, config: SingBoxConfig) {
        val deduplicatedConfig = applyDeletedNodeFilter(profile.id, deduplicateTags(config))
    ...
    private fun buildRunOutbounds(
        ...
        val deletedTags = profileId?.let { getDeletedNodeTags(it) } ?: emptySet()

        val fixedOutbounds = rawOutbounds?.mapNotNull { outbound ->
            if (outbound.type in PROXY_OUTBOUND_TYPES && outbound.tag in deletedTags) {
    ...
    fun createNode(
        ...
        try {
            ...
            // 用户手动新增同名节点时，自动解除该 tag 的删除屏蔽。
            removeNodeTagFromDeleted(profileId, finalTag)
            ...
            val newConfig = applyDeletedNodeFilter(profileId, deduplicateTags(SingBoxConfig(outbounds = newOutbounds)))
    ...
    fun deleteNode(nodeId: String): Boolean {
        val node = _nodes.value.find { it.id == nodeId } ?: _allNodes.value.find { it.id == nodeId } ?: return false
        val profileId = node.sourceProfileId
        val config = loadConfig(profileId) ?: return false

        // 记录删除屏蔽：订阅更新后同名节点会继续被过滤，不再“删了又回来”。
        markNodeTagDeleted(profileId, node.name)
        val newConfig = applyDeletedNodeFilter(profileId, config)
    ...
    fun renameNode(nodeId: String, newName: String) {
        ...
        removeNodeTagFromDeleted(profileId, newName)
        newConfig = applyDeletedNodeFilter(profileId, deduplicateTags(newConfig))
    ...
    fun updateNode(nodeId: String, newOutbound: Outbound) {
        ...
        var newConfig = config.copy(outbounds = newOutbounds)
        removeNodeTagFromDeleted(profileId, newOutbound.tag)
        newConfig = applyDeletedNodeFilter(profileId, deduplicateTags(newConfig))
    ...
    fun cleanup() {
        scope.cancel()
        com.kunbox.singbox.utils.RegionDetector.clearCache()
        nodeIdCache.clear()
        configCache.clear()
        profileNodes.clear()
        savedNodeLatencies.clear()
