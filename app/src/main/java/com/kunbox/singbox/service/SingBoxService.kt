package com.kunbox.singbox.service
...
import com.kunbox.singbox.ipc.SingBoxIpcHub
import com.kunbox.singbox.ipc.VpnStateStore
import com.kunbox.singbox.model.AppSettings
import com.kunbox.singbox.model.SingBoxConfig
import com.kunbox.singbox.repository.ConfigRepository
import com.kunbox.singbox.repository.LogRepository
import com.kunbox.singbox.repository.RuleSetRepository
import com.kunbox.singbox.repository.SettingsRepository
import com.kunbox.singbox.repository.TrafficRepository
import com.kunbox.singbox.core.BoxWrapperManager
import com.kunbox.singbox.core.ProbeManager
import com.kunbox.singbox.core.SelectorManager
import com.kunbox.singbox.service.network.NetworkManager
import com.kunbox.singbox.service.network.TrafficMonitor
import com.kunbox.singbox.service.notification.VpnNotificationManager
import com.kunbox.singbox.service.manager.ConnectManager
import com.kunbox.singbox.service.manager.SelectorManager as ServiceSelectorManager
import com.kunbox.singbox.service.manager.CommandManager
import com.kunbox.singbox.service.manager.CoreManager
import com.kunbox.singbox.service.manager.NetworkHelper
import com.kunbox.singbox.service.manager.PlatformInterfaceImpl
import com.kunbox.singbox.service.manager.ShutdownManager
import com.kunbox.singbox.service.manager.ScreenStateManager
import com.kunbox.singbox.service.manager.RouteGroupSelector
import com.kunbox.singbox.service.manager.ForeignVpnMonitor
import com.kunbox.singbox.service.manager.NodeSwitchManager
import com.kunbox.singbox.service.manager.BackgroundPowerManager
import com.kunbox.singbox.service.manager.ServiceStateHolder
import com.kunbox.singbox.model.BackgroundPowerSavingDelay
import com.kunbox.singbox.utils.L
import com.kunbox.singbox.utils.KernelHttpClient
...
class SingBoxService : VpnService() {
    ...
    // 启动管理器
    private val startupManager: com.kunbox.singbox.service.manager.StartupManager by lazy {
        com.kunbox.singbox.service.manager.StartupManager(this, this, serviceScope)
    }

    // 关闭管理器
    private val shutdownManager: com.kunbox.singbox.service.manager.ShutdownManager by lazy {
        com.kunbox.singbox.service.manager.ShutdownManager(this, cleanupScope)
    }
    ...
    companion object {
        ...
        const val ACTION_NETWORK_BUMP = "com.kunbox.singbox.action.NETWORK_BUMP"
     ...
     * StartupManager 回调实现
     */
    private val startupCallbacks = object : com.kunbox.singbox.service.manager.StartupManager.Callbacks {
        // 状态回调
    ...
    suspend fun hotSwitchNode(nodeTag: String): Boolean {
        ...
        try {
            ...
            when (val result = serviceSelectorManager.switchNode(nodeTag)) {
                is com.kunbox.singbox.service.manager.SelectorManager.SwitchResult.Success -> {
                    L.result("HotSwitch", true, "Switched to $nodeTag via ${result.method}")
                    requestNotificationUpdate(force = true)
                    return true
                }
                is com.kunbox.singbox.service.manager.SelectorManager.SwitchResult.NeedRestart -> {
                    L.warn("HotSwitch", "Need restart: ${result.reason}")
                    // 需要完整重启，返回 false 让调用者处理
                    return false
                }
                is com.kunbox.singbox.service.manager.SelectorManager.SwitchResult.Failed -> {
...
/**
    ...
    private fun startVpn(configPath: String) {
        ...
        startVpnJob = serviceScope.launch {
            ...
            when (result) {
                is com.kunbox.singbox.service.manager.StartupManager.StartResult.Success -> {
                ...
                is com.kunbox.singbox.service.manager.StartupManager.StartResult.Failed -> {
                    stopVpn(stopService = true)
                }
                is com.kunbox.singbox.service.manager.StartupManager.StartResult.NeedPermission -> {
                    updateServiceState(ServiceState.STOPPED)
                    stopSelf()
                }
                is com.kunbox.singbox.service.manager.StartupManager.StartResult.Cancelled -> {
    ...
    override fun onDestroy() {
        ...
        runCatching {
            ...
            nm.cancel(com.kunbox.singbox.service.notification.VpnNotificationManager.NOTIFICATION_ID)
            stopForeground(STOP_FOREGROUND_REMOVE)
        }
    ...
    override fun onRevoke() {
        ...
        // 记录日志，告知用户原因
        com.kunbox.singbox.repository.LogRepository.getInstance()
